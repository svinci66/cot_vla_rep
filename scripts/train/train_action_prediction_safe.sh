...
# VILA-U Action Prediction Training Script (Safe Version)
# 基于实战经验优化，避免服务器环境常见陷阱
# 改进点：
# 1. 支持直接PATH注入，绕过conda激活问题
# 2. 默认关闭危险的transformers patch同步
# 3. 强制Python日志即时输出
# 4. 优化8卡训练默认配置
...
# 🔥 关键改进1：强制Python日志即时输出（避免日志缓冲）
export PYTHONUNBUFFERED=1

# Editable configuration
CONDA_ENV_NAME=${CONDA_ENV_NAME:-"vila_env_fixed"}
CONDA_PATH_OVERRIDE=${CONDA_PATH_OVERRIDE:-""}  # 新增：直接注入PATH的环境路径
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}  # 默认8卡
SINGLE_GPU_MODE=${SINGLE_GPU_MODE:-False}  # 默认多卡模式

NUM_GPUS=${NUM_GPUS:-8}  # 默认8卡
BATCH_SIZE=${BATCH_SIZE:-160}  # 8卡优化：每卡20
ACC_STEP=${ACC_STEP:-2}
...
USE_DEEPSPEED=${USE_DEEPSPEED:-True}  # 8卡默认开启DeepSpeed
USE_HYBRID_ATTENTION=${USE_HYBRID_ATTENTION:-True}
ALLOW_FLASH_HYBRID_ATTENTION=${ALLOW_FLASH_HYBRID_ATTENTION:-False}
SYNC_TRANSFORMERS_PATCH=${SYNC_TRANSFORMERS_PATCH:-False}  # 🔥 关键改进2：默认关闭（避免权限问题）
...
# 🔥 关键改进3：支持直接PATH注入（绕过conda激活问题）
if [ -n "$CONDA_PATH_OVERRIDE" ]; then
    echo "=========================================="
    echo "🚀 Using direct PATH injection (bypassing conda activate)"
    echo "   Path: $CONDA_PATH_OVERRIDE"
    echo "=========================================="
    export PATH="$CONDA_PATH_OVERRIDE/bin:$PATH"

    # 验证Python可用性
    if ! command -v python >/dev/null 2>&1; then
        echo "Error: Python not found in $CONDA_PATH_OVERRIDE/bin" >&2
        exit 1
    fi
    python_version=$(python --version 2>&1)
    echo "✓ Python found: $python_version"

elif [ -z "${CONDA_PREFIX:-}" ] && command -v conda >/dev/null 2>&1; then
    echo "=========================================="
    ...
    echo "=========================================="
    echo "ℹ️  Using current environment (CONDA_PREFIX already set or conda not available)"
    echo "=========================================="
fi

# 🔥 关键改进4：只在明确启用时才同步patch（并检查权限）
if [ "$SYNC_TRANSFORMERS_PATCH" = "True" ] || [ "$SYNC_TRANSFORMERS_PATCH" = "true" ]; then
    echo "=========================================="
    echo "⚠️  SYNC_TRANSFORMERS_PATCH is enabled"
    echo "=========================================="
    site_pkg_path=$(python -c 'import site; print(site.getsitepackages()[0])')

    # 检查写入权限
    if [ ! -w "$site_pkg_path/transformers/" ]; then
        echo "❌ Error: No write permission to $site_pkg_path/transformers/" >&2
        echo "   This is a shared environment. Please set SYNC_TRANSFORMERS_PATCH=False" >&2
        exit 1
    fi

    if [ -d "./vila_u/train/transformers_replace/" ]; then
        echo "✓ Copying transformers patches to $site_pkg_path/transformers/"
        cp -r ./vila_u/train/transformers_replace/* "$site_pkg_path/transformers/"
    else
        echo "⚠️  Warning: ./vila_u/train/transformers_replace/ not found, skipping patch"
    fi
else
    echo "=========================================="
    echo "✓ SYNC_TRANSFORMERS_PATCH is disabled (safe mode)"
    echo "=========================================="
fi
